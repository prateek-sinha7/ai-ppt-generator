...
class TestCompositeKeyGeneration:
    ...
    def test_key_is_deterministic(self):
        ...
        key1 = _make_slide_json_key("AI in Healthcare", "healthcare", "hexaware_corporate", "abc123", "1.0.0")
        key2 = _make_slide_json_key("AI in Healthcare", "healthcare", "hexaware_corporate", "abc123", "1.0.0")
        assert key1 == key2
    ...
    def test_different_topics_produce_different_keys(self):
        key1 = _make_slide_json_key("Topic A", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        key2 = _make_slide_json_key("Topic B", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        assert key1 != key2

    def test_different_industries_produce_different_keys(self):
        key1 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        key2 = _make_slide_json_key("topic", "finance", "hexaware_corporate", "abc", "1.0.0")
        assert key1 != key2

    def test_different_themes_produce_different_keys(self):
        key1 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        key2 = _make_slide_json_key("topic", "healthcare", "hexaware_professional", "abc", "1.0.0")
        assert key1 != key2

    def test_different_provider_hashes_produce_different_keys(self):
        key1 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "hash_claude", "1.0.0")
        key2 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "hash_openai", "1.0.0")
        assert key1 != key2

    def test_different_prompt_versions_produce_different_keys(self):
        key1 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        key2 = _make_slide_json_key("topic", "healthcare", "hexaware_corporate", "abc", "1.1.0")
        assert key1 != key2

    def test_topic_normalisation_case_insensitive(self):
        """Topic is normalised to lowercase before hashing."""
        key1 = _make_slide_json_key("AI in Healthcare", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        key2 = _make_slide_json_key("ai in healthcare", "healthcare", "hexaware_corporate", "abc", "1.0.0")
        assert key1 == key2
...
class TestResearchCache:
    ...
    async def test_research_key_uses_topic_and_industry(self, cache_service, sample_research):
        """The cache key must incorporate both topic and industry."""
    ...
    async def test_cache_miss_returns_none(self, cache_service):
        with patch("app.services.presentation_cache.redis_cache") as mock_redis:
            ...
            result = await cache_service.get_slide_json(
                "topic", "healthcare", "hexaware_corporate", "hash", "1.0.0"
            )
            assert result is None
    ...
    async def test_set_slide_json_uses_correct_ttl(self, cache_service, sample_slide_json):
        with patch("app.services.presentation_cache.redis_cache") as mock_redis:
            ...
            await cache_service.set_slide_json(
                "topic", "healthcare", "hexaware_corporate", "hash", "1.0.0", sample_slide_json
            )
